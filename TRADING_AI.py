...
class EnhancedTradingAI:
    def __init__(self, trading_manager=None):
        ...
        self.trading_manager = trading_manager if trading_manager else TradingManager()
...
if __name__ == '__main__':
    # Criar uma instância para teste
    testing_ai = EnhancedTradingAI()
    
    # Testar algumas funcionalidades
    print("Testando análise de mercado...")
    market_data = testing_ai.get_market_data_for_symbol('R_50')
    analysis = testing_ai.analyze_market_conditions(market_data)
    print(f"Análise para R_50: {analysis}")
    
    print("\nTestando predição com configuração de usuário...")
    prediction = testing_ai.predict_with_user_config('R_50', 't', 5)
    print(f"Predição: {prediction}")
    
    print("\nTestando análise completa da IA...")
    full_analysis = testing_ai.full_ai_analysis(['R_50', 'R_25', 'R_100'])
    print(f"Análise completa: {full_analysis}")
